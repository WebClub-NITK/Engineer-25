      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            <div
  className={`${
    isMobile ? "" : "sticky top-40"
  } flex flex-col md:flex-row z-40 items-center max-w-xs ...`}
>

              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-white dark:text-white text-3d-light">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full text-white">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-white text-3d-light">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
  ...
  maxHeight: "100%"
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[3px] bg-gradient-to-t from-white via-white/60 to-transparent from-[0%] via-[10%] rounded-full dark:from-white dark:via-white/50"
          />
        </div>
      </div>
    </div>
